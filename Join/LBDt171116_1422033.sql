...
SELECT  o_titulo "Titulo", o_lancamento "Lançamento", o_preco "Preço" from SP_BuscaAVGLivro ('mirandela editora');
...
SELECT codlivro,titulo,COALESCE(lancamento,'-- Não lançado') "Lançamento", preco
FROM Livro
WHERE UPPER (titulo) LIKE UPPER ('%sistemas%')
ORDER BY preco DESC;
------------------------------------------------
set term^;
...
BEGIN
	FOR
		SELECT codlivro,titulo,COALESCE(lancamento,'-- Não lançado') "Lançamento", preco
		FROM Livro
		WHERE UPPER (titulo) LIKE UPPER (:i_titulo)
		ORDER BY preco DESC
...
COMMIT;
----------------------------------------------
SELECT * FROM SP_Busca_Livros ('sistemas')
