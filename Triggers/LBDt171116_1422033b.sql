...
/* --------------------------------    
01. Crie a tabela "Log_livro" com os mesmos atributos e tipos da tabela Livro (sem definir PKs e FKs), além dos atributos usuario (varchar (30)), tipo de transacao, tiptrans(char(01)) transacao (integer), data (date) e hora (time)
-------------------------------- */
...
BEGIN
  ...
  Values 	(coalesce(new.codlivro,old.codlivro), 
			 ...
			 NULL,					-- para o tiptrans, por enquanto
			 current_user, current_transaction, 
          	 current_date, current_time);
END^
set term;^
...
--------------------------------    
set term^;
Create or Alter Trigger TG03_Log_Livro_B
FOR    Livro
ACTIVE
BEFORE Insert or Delete
AS 
